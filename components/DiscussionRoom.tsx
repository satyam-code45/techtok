...
export default function DiscussionRoom({ tutorName }: { tutorName: string }) {
  const { user } = useUser();
  ...
  const tutor = tutors.find((t) => t.name.includes(tutorName));
  const greeting = tutor?.greeting.replace("{username}", username) || "Tutor not found";
...
const handleDisconnect = () => {
  ...
  // Wait a short moment to show the ending message, then redirect
  setTimeout(() => {
    router.push("/");
  }, 1500); // 1.5s delay
};
  ...
  return (
    <div className="bg-gray-100 h-[80vh] w-full max-w-3xl mx-auto mt-10 rounded-xl border dark:bg-gray-700 flex flex-col overflow-hidden">
      {/* Header */}
